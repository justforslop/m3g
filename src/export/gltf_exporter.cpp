...
GltfPaths GltfExporter::write(const decode::Decoded &decoded, const std::string &output_path, bool overwrite,
    ...
    return write(decoded.scene_ir, output_path, overwrite, png_compression_level);
}

GltfPaths GltfExporter::write(const scene::SceneIr &scene_ir, const std::string &output_path, bool overwrite,
                              int png_compression_level) const {
    gltf::GltfWriter writer;
    auto result = writer.write(scene_ir, output_path, overwrite, png_compression_level);
    GltfPaths paths;
